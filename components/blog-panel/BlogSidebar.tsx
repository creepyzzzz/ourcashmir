...
const NAV_ITEMS = [
    { label: 'Dashboard', href: '/blog-panel', icon: LayoutDashboard },
    { label: 'All Posts', href: '/blog-panel/posts', icon: FileText },
    { label: 'Create Post', href: '/blog-panel/posts/create', icon: PenTool },
    { label: 'Media Library', href: '/blog-panel/media', icon: ImageIcon },
    { label: 'Categories', href: '/blog-panel/categories', icon: Tags },
    { label: 'Tags', href: '/blog-panel/tags', icon: Tags },
    // { label: 'Analytics', href: '/blog-panel/analytics', icon: BarChart2 },
];

const BOTTOM_ITEMS = [
    //{ label: 'Settings', href: '/blog-panel/settings', icon: Settings },
];
...
export default function BlogSidebar({ collapsed: controlledCollapsed, onToggle, mobileOpen, onMobileClose }: SidebarProps) {
    ...
    const toggleSidebar = () => {
        ...
        localStorage.setItem('blog_sidebar_collapsed', String(newValue));
        onToggle?.(newValue);
    };
    ...
    const SidebarContent = ({ isMobile = false }: { isMobile?: boolean }) => (
        <>
            ...
            <div className={`h-14 lg:h-16 flex items-center justify-between px-3 lg:px-4 border-b border-brand-primary/5 ${isMobile ? 'pr-2' : ''}`}>
                <Link href="/blog-panel" className="flex items-center gap-2 group overflow-hidden" onClick={isMobile ? onMobileClose : undefined}>
            ...
            <nav className="flex-1 overflow-y-auto py-4 lg:py-6 px-2 lg:px-3 space-y-1">
                ...
                {NAV_ITEMS.map((item) => {
                    const isActive = pathname === item.href; // Simplified check
                ...
                {/* Logout in nav for mobile? No, it's at bottom usually */}
            </nav>
                    ...
                    {(isMobile || !isCollapsed) && (
                        <>
                            <div className="flex-1 min-w-0">
                                <p className="text-[10px] lg:text-xs font-medium text-brand-white truncate">Blog Poster</p>
                            </div>
                            <button
